This game uses Python's requests library to send a request to the NASA Near Earth Object API. It passes a start date, end date, and api key. The API then returns info that can be extracted into a list of objects with asteroid names (string), estimated diameters (float), and relative velocities (float). In the game, the player must dodge the asteroids (which have their names and relative velocities/diameters). The background of the game is a picture generated by the API by fetching the daily image from NASA's Astronomy Picture of the Day endpoint.
To run the game, you must obtain a free API key from api.nasa.gov. Save the key as NASA_API_KEY=your_key_here in a file named .env in the root directory.

To play: install the requirements and run main.py
